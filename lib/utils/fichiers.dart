...
class Fichiers {
  static const _canal = MethodChannel('bodycount/fichiers');

  /// Laisse choisir un fichier et rend le chemin d'une copie privée, ou
  /// null si rien n'a été choisi.
  static Future<String?> choisir() {
    return EtatVerrou.instance.retenir(
      () => _canal.invokeMethod<String>('ouvrir'),
    );
  }

  /// Propose [chemin] aux autres applications.
  static Future<void> partager(String chemin) {
    return EtatVerrou.instance.retenir(
      () => _canal.invokeMethod<bool>('partager', {'chemin': chemin}),
    );
  }
  ...
  static Future<bool> enregistrer(String chemin, String nom) async {
    final ok = await EtatVerrou.instance.retenir(
      () => _canal.invokeMethod<bool>('enregistrer', {
        'chemin': chemin,
        'nom': nom,
      }),
    );
